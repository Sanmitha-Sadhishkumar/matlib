...
__all__=['addinv', 'aprog', 'are_rec', 'area_cir', 'area_cube', 'area_ellipse', 'area_parallelogram',
         'area_rectangular_prism', 'area_sector', 'area_sq', 'area_torus', 'area_trapezium', 'area_tri',
         'aseries', 'composite', 'cpdint', 'csa_hcylinder', 'csa_hhemisphere', 'csa_scone', 'csa_scylinder',
...
def simint(p,n,r):
    if (((isinstance(p,int) and (isinstance(n,int))) and isinstance(r,int))==False:
        raise TypeError("Invalid inputs for type 'int'")
    else:
        amt=p*n*r
        return amt/100
...
def distance(x1,y1,x2,y2):
    if (((isinstance(x1,int) and (isinstance(y1,int))) and (isinstance(x2,int) and isinstance(y2,int))==False:
        raise TypeError("Invalid inputs for type 'int'")
    else:
        x,y=x2-x1,y2-y1
        disq=(x**2+y**2)
...
# cpdint() - gives the compound interest of the given inputs
# inputs - 4(principal amount, n, rate of interest, time)
# output - compound interest of the given
def cpdint(p,n,r,t):
    if (((isinstance(p,int) and (isinstance(n,int))) and (isinstance(r,int) and isinstance(t,int))==False:
        raise TypeError("Invalid inputs for type 'int'")
    else:
        a,b=1+(r/n),n*t
        return p*(a**b)
